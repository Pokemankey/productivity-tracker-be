...
export class AuthService {
    ...
    async signup(dto: SignupDto, deviceInfo: string) {
        ...
        if (exists) throw new BadRequestException("Email or username is already in use.");
    ...
    async refresh(dto: RefreshTokenDto) {
        ...
        if(storedToken.revokedAt) throw new UnauthorizedException("Invalid refresh token");
        ...
        return this.generateTokens(decoded.sub, decoded.email, storedToken.deviceInfo);
    }

    private async generateTokens(userId: string, email: string, deviceInfo: string | null) {
        const jti = uuid()
